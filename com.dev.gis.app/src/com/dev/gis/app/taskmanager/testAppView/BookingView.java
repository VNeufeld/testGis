...
public class BookingView extends TaskViewAbstract {
	...
	private void createControls(Composite composite) {
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL)
				.grab(true, false).applyTo(bookingGroup);

		// GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL).grab(true,
		// false).applyTo(createRequestId(composite));

		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL)
				.grab(true, false).applyTo(createPaypalGroup(composite));
		...
		final Button buttonVerify = new Button(groupButtons, SWT.PUSH
				| SWT.LEFT);
		buttonVerify.setText("Verify");
		buttonVerify.addSelectionListener(new AddVerifyListener());

		final Button buttonPayPal = new Button(groupButtons, SWT.PUSH
				| SWT.LEFT);
		buttonPayPal.setText("PayPal ");
		buttonPayPal.addSelectionListener(new AddPaypalListener());

		final Button buttonPayPalResult = new Button(groupButtons, SWT.PUSH
				| SWT.LEFT);
	...
	private Control createPaypalGroup(Composite parent) {
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.FILL)
				.grab(true, false).applyTo(paypalUrl);
	...
	private Control createRequestId(Composite parent) {
		...
		GridDataFactory.fillDefaults().align(SWT.BEGINNING, SWT.FILL)
				.hint(300, 16).grab(false, false).applyTo(bookingRequestId);
	...
	private Composite createBookingGroup(Composite parent) {
		...
		GridDataFactory.fillDefaults().align(SWT.BEGINNING, SWT.FILL)
				.grab(true, false).hint(300, 16).applyTo(priceText);

		label = new Label(groupStamp, SWT.NONE);
		label.setText("Car:");
		carDescription = new Text(groupStamp, SWT.BORDER | SWT.SINGLE);
		GridDataFactory.fillDefaults().align(SWT.BEGINNING, SWT.FILL)
				.grab(true, false).hint(300, 16).applyTo(carDescription);
		...
		GridDataFactory.fillDefaults().align(SWT.BEGINNING, SWT.FILL)
				.hint(300, 16).grab(false, false).applyTo(bookingId);

		return groupStamp;
	}

	private void showOffer(final Offer selectedOffer, List<Extra> extras) {
		...
		public void widgetSelected(SelectionEvent arg0) {
			...
			try {
				...
				BookingRequest request = BookingRequestCreator
						.createBookingRequest(selectedOffer, selectedExtras);
				...
				BookingResponse response = service.verifyOffers(request,
						selectedOffer);
				bookingRequestId
						.setText(String.valueOf(response.getRequestId()));
	...
	protected class AddPaypalListener extends AbstractListener {
		...
		public void widgetSelected(SelectionEvent arg0) {
			...
			try {
				...
				AdacVehicleHttpService service = serviceFactory
						.getAdacVehicleJoiService();
				PaypalSetCheckoutResponse response = service.getPaypalUrl(
						selectedOffer, bookingRequestId.getText());
				...
				if (response != null) {
					...
					if (response.getError() != null)
						...
						PayPalDialog mpd = new PayPalDialog(parent.getShell(),
								response.getPaypalUrl());
						if (mpd.open() == Dialog.OK) {
							PaypalDoCheckoutResponse response2 = service
									.getPaypalResult(selectedOffer,
											bookingRequestId.getText(),
											paypalToken.getText());
							if (response2 != null) {
	...
	protected class AddPaypalResultListener extends AbstractListener {
		...
		public void widgetSelected(SelectionEvent arg0) {
			...
			PaypalDoCheckoutResponse response = service.getPaypalResult(
					selectedOffer, bookingRequestId.getText(),
					paypalToken.getText());
			if (response != null) {
				if (response.getError() != null)
