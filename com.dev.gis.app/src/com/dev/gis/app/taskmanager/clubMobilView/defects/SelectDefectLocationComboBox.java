...
public class SelectDefectLocationComboBox extends DynamicComboBox{
	
	private static Logger logger = Logger.getLogger(SelectDefectLocationComboBox.class);
	
	private int[] dynValues ;
	
	public SelectDefectLocationComboBox(Composite parent, int size) {
	...
	protected String[] createDynamicItems() {
		...
		try {
			if ( ClubMobilModelProvider.INSTANCE.defectListResponse != null) {
				List<DefectLocation>  locClasses = ClubMobilModelProvider.INSTANCE.defectListResponse.getDefectLocations();
