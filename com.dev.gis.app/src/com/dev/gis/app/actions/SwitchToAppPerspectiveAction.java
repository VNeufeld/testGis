...
public class SwitchToAppPerspectiveAction extends Action {
	...
	public SwitchToAppPerspectiveAction(String text, IWorkbenchWindow window) {
		super(text);
		this.window = window;
		// The id is used to refer to the action in a menu or toolbar
		setId(ICommandIds.CMD_SWITCH_TO_APP_PERSPECTIVE);
		// Associate the action with a pre-defined command, to allow key
		// bindings.
		setActionDefinitionId(ICommandIds.CMD_SWITCH_TO_APP_PERSPECTIVE);
	...
	public void run() {
		...
		if (PlatformUI.getWorkbench() != null) {
			...
			IPerspectiveDescriptor descriptor = window.getWorkbench()
					...
					.findPerspectiveWithId(TestAppPerspective.ID);
			...
			try {
				wp.showView(TestAppView.ID, Integer.toString(0),
